...
function StaffDonation() {
  ...
  const handleReject = async (donation) => {
    ...
    setUpdatingId(donation.donationId);
    try {
      const updated = { ...donation, status: "Rejected" };
      await api.put(`/donation/${donation.donationId}`, updated);
      fetchDonations();
    } finally {
      setUpdatingId(null);
  ...
  return (
    <SidebarLayout title="Xử lý đơn hiến máu" isStaff>
      <div style={{ padding: 32 }}>
        <h2 style={{ color: '#174c8f', marginBottom: 24 }}>Xử lý đơn hiến máu các thành viên</h2>
            ...
            <tbody>
              {donations.map(d => (
                <tr key={d.donationId} style={{ background: '#f9fafb' }}>
                  ...
                  <td style={tdStyle}>{d.member?.fullName || d.member?.memberId || ''}</td>
                  <td style={tdStyle}>{d.location?.name || d.location?.locationId || ''}</td>
                  <td style={tdStyle}>{d.date}</td>
                  <td style={tdStyle}>{d.notes}</td>
                  <td style={tdStyle}>{d.status}</td>
                  <td style={tdStyle}>
                    <Link to={`/staff/donation/${d.donationId}`} style={{color:'#1976d2',textDecoration:'underline',marginRight:8}}>Xem chi tiết</Link>
                    {(d.status === 'Pending' || d.status === 'Chờ duyệt') && (
                      <>
                        <button onClick={() => handleApprove(d)} disabled={updatingId === d.donationId} style={{background:'#388e3c',color:'#fff',border:'none',borderRadius:8,padding:'6px 14px',marginRight:8,cursor:updatingId===d.donationId?'not-allowed':'pointer'}}>Duyệt</button>
                        <button onClick={() => handleReject(d)} disabled={updatingId === d.donationId} style={{background:'#d32f2f',color:'#fff',border:'none',borderRadius:8,padding:'6px 14px',cursor:updatingId===d.donationId?'not-allowed':'pointer'}}>Từ chối</button>
                      </>
                    )}
